...
from .forms import DocumentForm, GuardianForm, PromotionForm, StudentForm
from .models import Student, StudentDocument, StudentGuardian
from .services import import_students, promote
...
class StudentCreateView(ERPCreateView):
    ...
    def get_initial(self):
        return {
            "student_id": Student.next_student_id(self.request.school, str(timezone.localdate().year)),
...
def import_csv(request):
    ...
    class UploadForm(TailwindFormMixin, forms.Form):
        file = forms.FileField(
            help_text="UTF-8 CSV: student_id, first_name, last_name, gender (M/F/O), date_of_birth, admission_date, status."
        )
...
def export(request):
    ...
    return spreadsheet(
        "students",
        ["ID", "Name", "Status"],
        [[s.student_id, s.full_name, s.status] for s in students],
...
def id_card(request, pk):
    ...
    return pdf_response(
        "Student ID card",
        ["School", "ID", "Student", "Class"],
        [
            [
                request.school.name,
                s.student_id,
                s.full_name,
                str(s.current_enrollment.section) if s.current_enrollment else "",
            ]
        ],
    )
