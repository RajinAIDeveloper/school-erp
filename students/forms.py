...
from .models import Student, Guardian, Enrollment, StudentGuardian, StudentDocument

class StudentForm(SchoolModelForm):
    class Meta:
        model = Student
        fields = ["student_id", "user", "first_name", "last_name", "name_bn", "gender",
                  "date_of_birth", "birth_registration_no", "blood_group", "religion", "phone",
                  "email", "photo", "present_address", "permanent_address", "previous_school",
...
class PromotionForm(TailwindFormMixin, forms.Form):
    ...
    def clean(self):
        data = super().clean()
        if data.get("source_year") and data.get("target_year") and data["target_year"].start_date <= data["source_year"].start_date:
