...
A file is judged by what is in it, never by its name: a PDF, a photo (JPEG, PNG or WebP) or a
Word document, and nothing else. Photos are redrawn at a sensible size as JPEG, which also
drops the camera's EXIF data, the phone's location with it. Files are stored under a random
name and served only through a checked view, never from a public address.
"""
...
def prepare(upload):
    ...
    redrawn as JPEG. Anything that is not a PDF, a photo or a Word document is refused.
    """
    ...
    upload.seek(0)
    if kind == "zip":
        kind = "docx" if _is_word_document(upload) else None
        upload.seek(0)
    if kind is None:
        raise ValidationError(gettext("%(name)s is not a PDF, a photo or a Word document.") % {"name": _name(upload)})
    if kind in ("jpeg", "png", "webp"):
        return ContentFile(_redraw(upload), name="page.jpg"), "jpeg"
