...
| P4 | Transcript; online admissions (module) | Done: 4a, 4b and 4c |
| P4a | Transcript: families request it in the portal, the Principal, Vice Principal or Administrator issues it | Done: test_transcripts.py. Each year's grades as awarded with the grading keys used, confirmed and released official results, approved predicted grades; numbered and frozen, with serial, fingerprint and page n of N on every page; the public check shows the grades with initials only and says when a result on it has since been corrected (a classmate's correction does not count) |
| P4b | Online admissions foundation (module): rounds, the public application, the family's private page, office screens, fee at the office | Done: test_admissions_public.py, test_admissions_office.py, test_admissions_module.py. A family applies without an account (honeypot, time trap, rate limits, versioned consent, date-of-birth window to the day) and gets a number, a private link (only its hash is stored; it moves into the session) and a slip with a QR code; their page shows the status, what is missing, the fee and any offer, in English or Bangla. Staff see each class's applications against its seats, check documents, confirm siblings, flag duplicates without refusing them, enter walk-ins and give a new link. The fee is receipted at the office and posted to Admission Fee Income, voided only from the application. Offering and turning down are the managers' |
| P4c | Assessment, merit list, offers and waitlist, enrolment, funnel, purging old applications | Done: test_admissions_selection.py, test_admissions_enrolment.py. Tests and interviews in sittings with a capacity (the fee first where the round asks), families see their booking, scores on a grid checked before saving. The merit list comes out the same from the same results: the pass mark, the score, the round's sibling rule (none, tie-break or priority), then who applied first; fixing it is audited, offers follow it and never exceed the seats, a waiting list moves up when a seat comes free or an offer lapses. Enrolment through `admit()`: a guardian with the same number only once staff confirm the family, documents and photo copied, consent carried over, a warning for a year that is not current, the welcome SMS only where the school switched it on. Funnel report with export. `admissions_purge` clears applications after the keeping period, keeping anonymous counts; erasure clears the application too. Demo round in `seed_demo` |
