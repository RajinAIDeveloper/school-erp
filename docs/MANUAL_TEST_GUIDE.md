...
- [ ] Click **Basic Settings → Classes → Add**. Enter **Name** `Class 1`, **Order** `1`, and choose the school's own assessment rules for the first test. Save. Click **Sections → Add**; select Class 1, enter `A`, capacity `30`, leave Class teacher blank until the teacher exists. **Expected:** `Class 1 - A` appears.
...
- [ ] For **Teacher** and **Staff**, first create their employee records in section 4, then click **Create login** on each staff file. For **Student** and **Guardian**, first admit and link the child and guardian, then click **Create login** on their records. **Expected:** each login is tied to the correct person and receives its matching role automatically. Temporary passwords appear **once**, so copy them before leaving that page; the person must change the password at first sign-in. Giving someone the Teacher role on **User Management** alone does not create an employee record or put them in the **Class teacher** list.

## 4. Put people into the school (Administrator)

### 4.1 Employee, section teacher and timetable

**If Class teacher shows only dashes:** first add the person under **Teachers & Staff → Add** with **Type: Teacher** and **Status: Active**. The section list reads employee records in this school, not usernames in User Management. If you already made a Teacher user first, create the employee record but **do not click Create login**, because that would make a second account. Ask the platform superuser to open **Django Admin → Employees → Employees**, edit that employee, set **User** to the existing account, and save. Then return to **Basic Settings → Sections** and select the employee as Class teacher. The link is needed for that teacher's attendance and homework permissions.

- [ ] Sign in as `test_admin`. Click **Teachers & Staff → Add**. Create a teacher with employee ID `T-001`, name `Test Teacher`, a safe example phone, joining date `2026-01-01`, type **Teacher**, department **Academics** and designation **Assistant Teacher** (create those two under Basic Settings if needed). **Expected:** the employee detail page opens.
- [ ] On that employee page click **Create login**. **Expected:** a username and temporary password appear once. Save them privately. Sign in as that teacher once and set a new password if prompted.
...
- [ ] Click **Teachers & Staff → Add** again. Make employee `S-001`, name `Test Staff`, type **Staff**, then create its login from its file. **Expected:** the Staff account can see **My staff file** and **Leave**, but not teacher mark entry.
