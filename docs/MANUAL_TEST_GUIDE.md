...
- [ ] Open **Routine → School days & periods** (also available from **Basic Settings → Overview → School week and periods**). Tick the days the school opens, then click **Save school days**. The other days disappear from the routine and do not require attendance.
- [ ] On that page, choose **Default schedule**, **Morning**, **Day**, or **Evening**. Under **Build the day**, enter the first start time (for example `08:30` or `14:00`), minutes per lesson (for example `40`), number of lessons (for example `6`), and any breaks. Click **Build the day**. **Expected:** the periods and their exact start and end times appear above. These times belong to this school and this schedule; the original 09:00 periods are only starter values. If this schedule already has lessons, use **Edit** on individual periods instead of replacing it.
- [ ] If one weekday is shorter, choose that weekday under **A day with other timings**. Change a period's start/end time or tick **Not held**, then save. **Expected:** that weekday shows its own times; other weekdays keep the usual schedule. For two shifts, build Morning and Evening separately, then open **Basic Settings → Sections** and set each section's **Shift**. **Routine → Edit week** will show the periods for that section's shift.
...
**New optional subject setup flow:** after you save a subject row, the ERP opens **Set up [subject] for [class]** instead of sending you back to the row list. From there you can **Assign teacher** for each section, **Set weekly routine**, add dated **term topics and learning goals**, plan future **quiz/class-test** dates, create a graded exam paper, and schedule homework if your school has Homework. You can skip any of these and click **Finish for now**. No teaching-plan entries are required for attendance, routine, marks or homework to work. To return later, open **Teaching plans → choose year and class → Continue setup** on the subject row.

- [ ] For a Cambridge example, add **Mathematics** to **2026 / Class 7**, leave **Group** blank, and choose **Compulsory**. On the next page, assign the Mathematics teacher to **Class 7–A**, then set Mathematics on two days under **Set weekly routine**. Under **Optional term teaching plan**, add a dated **Fractions** lesson with topic **Adding fractions**. Add a **Quiz planned** item for a future day if you want a reminder; to record marks, separately create an exam named **Math Quiz 1** and add a dated Class 7 Mathematics paper. **Expected:** the setup page shows the teacher, weekly periods, planned topic and scheduled paper. If you leave the optional teaching plan empty, the rest still works.
...
**Department** means the team, such as **Academics**. **Designation** means the job title, such as **Assistant Teacher**. Both are optional. If their dropdowns show only dashes, either leave them blank and save, or click **Teachers & Staff → Departments → + New** to add Academics and **Teachers & Staff → Designations → + New** to add Assistant Teacher. Being a class teacher is a separate assignment under **Basic Settings → Sections**.

**Qualification** is also optional: leave it blank or write a short summary such as **B.Ed**. On the same employee form, you can optionally upload a **CV**, an **Academic document** (degree or certificate), and an **Other document**. Use a PDF, photo or DOCX file up to 10 MB each. After saving, open the employee's page and click **Upload document** or **Add another** to attach more files; choose the document type, give it a title, select the file, then save. The files appear under **Documents** on that employee's page.
...
- [ ] Click **Routine → School days & periods → Rooms** and add `Classroom A` and `Gym` if they are not there yet. Click **Basic Settings → Sections**, edit `Class 1 - A`, set **Class teacher** to Test Teacher and **Default room** to `Classroom A`, then save. A new section has the same Default room choice on its creation form. Then **Subject teachers → Add**: choose 2026, Class 1 - A, Mathematics, Test Teacher. Repeat for English if you want the same teacher to mark both. **Expected:** the teacher sees only these assigned sections/subjects.
...
- [ ] Click **Routine → Edit week**. Pick year 2026 and section Class 1 - A. Put Mathematics with Test Teacher in one teaching period. Leave Room on **Section classroom: Classroom A** and save. **Expected:** the routine shows Classroom A even though no separate room was entered on the lesson. For a Gym lesson, choose **Gym** in that period's room dropdown; it changes only that repeating lesson. Try assigning the same teacher or room to another class at the same time; **Expected:** a clash is refused.
- [ ] To move the whole section's classroom for a day or week, open **Routine → Temporary classroom changes → + New**. Select `Class 1 - A`, start `2026-09-28`, end `2026-10-02`, and room `Gym` or another spare classroom; save. On **Routine**, enter a date in **Week containing date** and click **Show**. **Expected:** ordinary lessons show the temporary room for those dates; a lesson explicitly set to Gym/Lab keeps its own room. Leave the date blank to see the regular weekly routine. Edit the temporary change to adjust its dates or room.
