...
[docs/access-matrix.md](docs/access-matrix.md) shows exactly who can open what; it is
generated by `python manage.py role_matrix` from the URLs and the groups, so it cannot drift
from the code. Three endpoints are reachable without signing in — a Public download, report-card
verification, and the health probe — and a test asserts that list stays exactly that.
