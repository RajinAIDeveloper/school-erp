...
from core.models import audit,School
from .models import Account,JournalEntry,Payroll,record_simple_entry,ensure_default_accounts
...
def journal_create(request):
    ...
    return render(request,"generic/form.html",{"form":form,"page_title":"Post journal · income, expense or transfer"})
...
def reverse_entry(request,pk):
    ...
    if entry.source in ("fee","salary"):
        messages.error(request,"Reverse fee payments through the collection desk; paid payroll corrections require an adjustment journal.")
...
def report(request):
    ...
    rows=[[a.code,a.name,a.account_type,a.balance(start,end)] for a in Account.objects.filter(school=request.school)]
    ...
    return render(request,"generic/report.html",{"page_title":"Account balances / income and expenses","form":form,"headers":headers,"rows":rows})

@require_permission("finance.view_payroll")
def payroll(request):
    return render(request,"finance/payroll.html",{"rows":Payroll.objects.filter(school=request.school).select_related("employee"),"page_title":"Payroll"})
...
def payroll_pay(request,pk):
    with transaction.atomic():
        ...
        if not row.journal_entry_id:
            ...
            row.journal_entry=record_simple_entry(request.school,timezone.localdate(),str(row),
                Account.objects.get(school=request.school,code="5010"),Account.objects.get(school=request.school,code="1010"),
                row.net,source="salary",user=request.user)
...
def payslip(request,pk):
    ...
    return pdf_response("Payslip",["Employee","Month","Basic","Allowance","Deduction","Net","Paid on"],
        [[p.employee.full_name,p.month,p.basic,p.allowance,p.deduction,p.net,p.paid_on]],request.school.name)
