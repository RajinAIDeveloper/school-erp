...
from examinations.services import save_mark,publish_exam

SCREENS={
    "dashboard":"/","students":"/students/","employees":"/employees/","student_attendance":"/attendance/",
    "staff_attendance":"/attendance/staff/","fees":"/fees/","accounts":"/finance/","results":"/exams/",
    "routine":"/routine/","downloads":"/downloads/","sms":"/sms/","holidays":"/holidays/",
    "settings":"/settings/","users":"/users/","reports":"/reports/","overview":"/reports/overview/",
}
ACCESS={
    "Administrator":{"dashboard","students","employees","student_attendance","staff_attendance","fees","accounts","results","routine","downloads","sms","holidays","settings","users","reports","overview"},
    "Principal":{"dashboard","students","employees","student_attendance","staff_attendance","fees","accounts","results","routine","downloads","sms","holidays","users","reports","overview"},
    "Accountant":{"dashboard","students","employees","fees","accounts","downloads","sms","holidays","reports","overview"},
    "Teacher":{"dashboard","students","employees","student_attendance","staff_attendance","results","routine","downloads","holidays","reports"},
...
def test_role_screen_matrix(erp,role):
    user={
        "Administrator":erp.admin,"Teacher":erp.teacher,"Accountant":erp.accountant,
        "Staff":erp.staff,"Guardian":erp.parent
    }.get(role)
    if not user:
...
def test_teacher_can_access_assigned_student_and_marks_but_not_other(erp):
    ...
    assert b"Select a valid choice" in c.get(f"/exams/marks/?schedule={erp.schedule.pk}&section={erp.other_section.pk}").content
...
def test_staff_attendance_is_own_and_leave_is_own(erp):
    ...
    other=Employee.objects.create(school=erp.school,employee_id="S2",employee_type="staff",first_name="Other",gender="M",phone="01712345670",joining_date=date(2026,1,1))
    mine=Employee.objects.create(school=erp.school,user=erp.staff,employee_id="S1",employee_type="staff",first_name="Mine",gender="M",phone="01712345671",joining_date=date(2026,1,1))
    c=Client()
...
def test_parent_portal_rejects_other_child_query_and_shows_only_published_results(erp,invoice):
    ...
    other=Student.objects.create(school=erp.school,student_id="PRIVATE",first_name="Private",gender="M",
        date_of_birth=date(2016,1,1),admission_date=date(2026,1,1))
    StudentAttendance.objects.create(school=erp.school,enrollment=erp.enrollment,date=date(2026,9,21),status="present")
    c=Client()
...
def test_report_overview_counts_exports_and_permissions(erp):
    ...
    result=generate_invoices(school=erp.school,user=erp.accountant,academic_year=erp.year,class_level=erp.level,
        month=9,issue_date=date(2026,9,1),due_date=date(2026,9,10))
    assert result==(1,0)
    inv=FeeInvoice.objects.get()
    collect_payment(school=erp.school,user=erp.accountant,invoice=inv,amount=300,method="cash",reference="",date=date(2026,9,21))
...
def test_holiday_calendar_export_is_real_ical(erp):
    ...
    Holiday.objects.create(school=erp.school,name="Autumn break",start_date=date(2026,10,1),end_date=date(2026,10,2))
