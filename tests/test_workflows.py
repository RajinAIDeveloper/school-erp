...
from fees.services import generate_invoices,collect_payment,cancel_payment
from fees.models import FeeInvoice,FeePayment
from finance.models import Account,JournalEntry,record_simple_entry
from examinations.models import Mark,ResultSnapshot,UnlockRequest,GradeRule
from examinations.services import save_mark,publish_exam,build_result_sheet,review_unlock
from attendance.services import save_register,monthly_matrix
...
from students.services import promote,import_students
...
def pay(erp,invoice,amount=400):
    return collect_payment(school=erp.school,user=erp.accountant,invoice=invoice,amount=amount,method="cash",reference="",date=date(2026,9,21))

def test_fee_generation_is_idempotent(erp,invoice):
    result=generate_invoices(school=erp.school,user=erp.admin,academic_year=erp.year,class_level=erp.level,
...
def test_overpayment_rolls_back(erp,invoice):
    with pytest.raises(ValidationError): pay(erp,invoice,1001)
...
def test_cross_school_payment_rejected(erp,invoice):
    with pytest.raises(PermissionDenied):
        collect_payment(school=erp.other,user=erp.admin,invoice=invoice,amount=100,method="cash",reference="",date=date.today())
...
def test_cross_school_account_rejected(erp):
    ...
    with pytest.raises(PermissionDenied):
        record_simple_entry(erp.school,date.today(),"Wrong school",external,Account.objects.get(school=erp.school,code="4090"),100,source="manual")

def test_publish_requires_complete_marks(erp):
    with pytest.raises(ValidationError): publish_exam(erp.exam,erp.admin)
    assert not ResultSnapshot.objects.exists()

def test_mark_validation_and_conflict(erp):
    with pytest.raises(ValidationError): save_mark(user=erp.teacher,schedule=erp.schedule,enrollment=erp.enrollment,score=Decimal(101))
    mark=save_mark(user=erp.teacher,schedule=erp.schedule,enrollment=erp.enrollment,score=Decimal(80))
    with pytest.raises(ValidationError):
        save_mark(user=erp.teacher,schedule=erp.schedule,enrollment=erp.enrollment,score=Decimal(90),expected_version=0)
...
def test_published_snapshot_immutable_and_lock(erp):
    ...
    with pytest.raises(ValidationError):
        save_mark(user=erp.admin,schedule=erp.schedule,enrollment=erp.enrollment,score=Decimal(90),expected_version=1)
...
def test_scoped_unlock_new_snapshot_keeps_history(erp):
    ...
    exam=publish_exam(erp.exam,erp.admin)
    unlock=UnlockRequest.objects.create(school=erp.school,schedule=erp.schedule,requested_by=erp.teacher,reason="Correction")
...
def test_expired_unlock_rejected(erp):
    ...
    UnlockRequest.objects.create(school=erp.school,schedule=erp.schedule,requested_by=erp.teacher,reason="Old",
                                status="approved",expires_at=timezone.now()-timedelta(seconds=1))
    with pytest.raises(ValidationError):
        save_mark(user=erp.teacher,schedule=erp.schedule,enrollment=erp.enrollment,score=Decimal(80),expected_version=1)

def test_incomplete_absent_and_ranks(erp):
    from examinations.services import assign_ranks
    rows=[{"total":"90","complete":True},{"total":"90","complete":True},{"total":"80","complete":True},{"total":"99","complete":False}]
    assign_ranks(rows)
...
def test_attendance_holiday_and_duplicate(erp):
    ...
    with pytest.raises(ValidationError): save_register(**data)

def test_attendance_roster_permission(erp):
    erp.enrollment.section=erp.other_section
    erp.enrollment.save()
    with pytest.raises(PermissionDenied):
        save_register(school=erp.school,user=erp.teacher,day=date(2026,9,21),entries=[(erp.enrollment,"present","",None,None)])
...
def test_enrollment_cannot_use_other_school(erp):
    ...
    form=EnrollmentForm({"student":erp.student.pk,"academic_year":erp.year.pk,"class_level":foreign.pk,
                         "section":erp.section.pk,"roll_number":2,"status":"enrolled"},school=erp.school)
    assert not form.is_valid() and "class_level" in form.errors

def test_duplicate_enrollment_is_form_error(erp):
    form=EnrollmentForm({"student":erp.student.pk,"academic_year":erp.year.pk,"class_level":erp.level.pk,
                         "section":erp.section.pk,"roll_number":1,"status":"enrolled"},school=erp.school)
    assert not form.is_valid()

def test_promotion_keeps_history(erp):
    year=AcademicYear.objects.create(school=erp.school,name="2027",start_date=date(2027,1,1),end_date=date(2027,12,31))
...
def test_invalid_import_is_atomic(erp):
    upload=SimpleUploadedFile("students.csv",b"student_id,first_name,gender,date_of_birth,admission_date,status\nS2,Valid,M,2016-01-01,2026-01-01,active\nS3,Invalid,X,wrong,2026-01-01,active")
    with pytest.raises(ValidationError): import_students(erp.school,upload)
...
def test_timetable_overlap_with_different_period(erp):
    ...
    overlap=Period.objects.create(school=erp.school,name="Two",order=2,start_time=time(9,30),end_time=time(10,30))
    RoutineSlot.objects.create(school=erp.school,academic_year=erp.year,section=erp.section,weekday=1,period=first,subject=erp.subject,teacher=erp.employee)
    slot=RoutineSlot(school=erp.school,academic_year=erp.year,section=erp.other_section,weekday=1,period=overlap,subject=erp.subject,teacher=erp.employee)
    with pytest.raises(ValidationError): slot.full_clean()

def test_sms_queues_and_deduplicates_without_sending(erp):
    batch=queue_batch(erp.school,"Test","custom","Hello",[("One","01712345678"),("Same","+8801712345678")],erp.admin)
...
def test_sms_failure_logged(erp):
    ...
    class Failure(BaseSMSBackend):
        def send(self,message): raise RuntimeError("Gateway down")
...
def test_parent_cannot_read_other_student_or_draft_results(client,erp,invoice):
    ...
    other=Student.objects.create(school=erp.school,student_id="S2",first_name="Private",gender="M",date_of_birth=date(2016,1,1),admission_date=date(2026,1,1))
...
def test_cross_school_object_edit_denied(admin_client,erp):
    other=Student.objects.create(school=erp.other,student_id="S1",first_name="Private",gender="M",date_of_birth=date(2016,1,1),admission_date=date(2026,1,1))
...
def test_user_password_hashed_and_roles(admin_client,erp):
    ...
    r=admin_client.post("/users/new/",{"username":"new-user","password":"Unique-Strong-249!","roles":[group.pk],"is_active":"on"})
...
def test_published_mark_database_trigger_blocks_update_delete_insert(erp):
    ...
    for operation in (
        ...
        lambda: Mark.objects.create(school=erp.school,schedule=erp.schedule,enrollment=erp.enrollment,marks_obtained=99),
    ):
...
def test_published_exam_schedule_edit_rejected(admin_client,erp):
    save_mark(user=erp.teacher,schedule=erp.schedule,enrollment=erp.enrollment,score=Decimal(70))
    publish_exam(erp.exam,erp.admin)
    r=admin_client.post(f"/exams/schedule/{erp.schedule.pk}/edit/",{"exam":erp.exam.pk,"class_level":erp.level.pk,"subject":erp.subject.pk,"full_marks":50,"pass_marks":20})
...
def test_download_permissions_and_private_media(client,erp,settings,tmp_path):
    ...
    item=DownloadItem.objects.create(school=erp.school,category=cat,title="Staff only",
        audience="staff",file=SimpleUploadedFile("staff.txt",b"private staff content"))
    client.force_login(erp.parent)
...
def test_create_student_and_duplicate_validation(admin_client,erp):
    data={"student_id":"S2","first_name":"New","gender":"M","date_of_birth":"2016-01-01","admission_date":"2026-01-01","status":"active"}
...
def test_user_edit_cannot_escalate_to_platform_superuser(admin_client,erp):
    response=admin_client.post(f"/users/{erp.teacher.pk}/edit/",
        {"username":"teacher","roles":[Group.objects.get(name="Teacher").pk],"is_active":"on","is_superuser":"on","is_staff":"on"})
    assert response.status_code==302
...
def test_student_post_cannot_link_cross_school_user(admin_client,erp):
    ...
    response=admin_client.post("/students/new/",{"student_id":"S2","user":user.pk,"first_name":"New","gender":"M","date_of_birth":"2016-01-01","admission_date":"2026-01-01","status":"active"})
    assert response.status_code==200
...
def test_payroll_posts_once_and_receipt_pdf(admin_client,erp,invoice):
    ...
    row=Payroll.objects.create(school=erp.school,employee=erp.employee,month=date(2026,9,1),basic=1000,allowance=100,deduction=50)
    for _ in range(2):
...
def test_invalid_mark_id_returns_400(admin_client,erp):
    response=admin_client.post("/exams/marks/save/",{"schedule":erp.schedule.pk,"section":erp.section.pk,"enrollment":"bad","score":50,"expected_version":0})
