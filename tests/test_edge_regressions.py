...
from holidays.models import Holiday,is_holiday,holiday_dates_between
...
def test_holidays_exclude_weekend_and_open_event(erp):
    ...
    Holiday.objects.create(school=erp.school,name="Sports day",holiday_type="event",closes_school=False,start_date=monday,end_date=monday)
    assert not is_holiday(erp.school,monday)
...
def test_public_download_and_tenant_denial(erp,settings,tmp_path):
    ...
    item=DownloadItem.objects.create(school=erp.school,category=cat,title="Prospectus",
        audience="public",file=SimpleUploadedFile("prospectus.pdf",b"%PDF-sample"))
    c=Client()
...
def test_download_audience_restricts_unlinked_guardian(erp,settings,tmp_path):
    ...
    item=DownloadItem.objects.create(school=erp.school,category=cat,title="Class 2 file",
        audience="students",file=SimpleUploadedFile("class2.txt",b"restricted"))
...
def test_sms_http_gateway_failure_is_recorded_without_real_network(erp,settings):
    ...
    class FakeResponse:
        ...
        def read(self): return b'{"response_code":"403"}'
...
def test_manual_journal_reversal_preserves_old_period(erp):
    ...
    if timezone.localdate().month!=8:
        assert cash.balance(date(2026,8,1),date(2026,8,31))==100
    with pytest.raises(ValidationError): posted.post()

def test_payroll_negative_net_rejected_on_form(erp):
    from finance.views import payroll_create
    c=Client()
    c.force_login(erp.admin)
    r=c.post("/finance/payroll/new/",{"employee":erp.employee.pk,"month":"2026-09-01","basic":"100","allowance":"0","deduction":"200"})
    assert r.status_code==200
...
def test_account_cycle_rejected_on_form(erp):
    ...
    form=Form({"code":parent.code,"name":parent.name,"account_type":parent.account_type,"parent":child.pk},instance=parent,school=erp.school)
...
def test_subject_analysis_grading_bounds(erp):
    ...
    with pytest.raises(ValidationError): rule.full_clean()
