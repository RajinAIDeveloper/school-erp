...
from core.access import require_permission, sections_for, is_manager
from core.forms import TailwindFormMixin, SchoolModelForm
from core.exports import spreadsheet
from core.models import audit
from employees.models import Employee
from .models import StudentAttendance, StaffAttendance, AttendanceStatus, LeaveRequest
from .services import save_register, enrollments_for, monthly_matrix
...
def register(request,staff=False):
    ...
    editable=request.user.has_perm("attendance.change_staffattendance" if staff else "attendance.change_studentattendance")
    ...
    if form.is_valid():
        ...
        for obj in objects:
            ...
            initial={key:getattr(record,key,None) for key in ("status","remarks","check_in","check_out")} if record else {}
    ...
    return render(request,"attendance/take.html",{"filter_form":form,"rows":rows,"staff":staff,"editable":editable,"page_title":"Staff attendance" if staff else "Student attendance"})
...
def report(request,staff=False):
    ...
    if not request.user.has_perm(permission):
        raise PermissionDenied
    form=RegisterFilter(request.GET or {"date":timezone.localdate()},user=request.user,school=request.school,staff=staff)
    days,rows=[],[]
    if form.is_valid():
        day=form.cleaned_data["date"]
        if staff:
            ...
            objects=Enrollment.objects.filter(school=request.school,section=section,
                academic_year__start_date__lte=month_end,academic_year__end_date__gte=month_start).select_related("student","academic_year")
        days,rows=monthly_matrix(request.school,list(objects),day.year,day.month,staff)
        if request.GET.get("format") in ("csv","xlsx"):
            return spreadsheet("attendance",["Name",*[str(d.day) for d in days],"Present","Working days","Percent"],
                               [[r["name"],*r["cells"],r["present"],r["working"],r["pct"]] for r in rows],request.GET["format"])
    return render(request,"attendance/report.html",{"filter_form":form,"rows":rows,"days":days,"page_title":"Monthly attendance report"})
...
def leaves(request):
    ...
    return render(request,"attendance/leaves.html",{"rows":qs,"page_title":"Leave requests","can_review":is_manager(request.user)})
