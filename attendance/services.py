...
from core.access import assert_school, sections_for, is_manager, assert_actor_school
...
def enrollments_for(section, day):
    return Enrollment.objects.filter(school=section.school, section=section,
        academic_year__start_date__lte=day, academic_year__end_date__gte=day,
        student__admission_date__lte=day).exclude(status="left").select_related("student","academic_year")
...
def save_register(*, school, user, day, entries, staff=False):
    ...
    for obj, status, remarks, check_in, check_out in entries:
        ...
        AuditLog.objects.create(school=school,user=user,action="attendance.saved",
            model=record._meta.label,object_id=str(record.pk),description=f"{day}: {status}")
        saved += 1
    return saved
...
def monthly_matrix(school, objects, year, month, staff=False):
    ...
    records={(getattr(a,field),a.date):a.status for a in model.objects.filter(
    ...
    for obj in objects:
        ...
        rows.append({"name":str(obj) if staff else obj.student.full_name,"object":obj,
                     "cells":[records.get((obj.pk,d),"?") for d in days],"present":present,
                     "working":len(working),"pct":round(present*100/len(working),1) if working else None})
    return days,rows
