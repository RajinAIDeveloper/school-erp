...
def queue_batch(school,title,recipients_kind,body,contacts,user=None):
    ...
    for name,phone in contacts:
        if phone: recipients[normalize_bd_phone(phone)]=name
    if not recipients:
    ...
    SMSMessage.objects.bulk_create([SMSMessage(school=school,batch=batch,recipient_name=name,phone=phone,body=body) for phone,name in recipients.items()])
    return batch
