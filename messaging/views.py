...
class ComposeForm(TailwindFormMixin,forms.Form):
    ...
    numbers=forms.CharField(widget=forms.Textarea,required=False,help_text="One Bangladeshi mobile number per line.")
...
def compose(request):
    ...
    if request.method=="POST" and form.is_valid():
        ...
        if d["recipients"]=="custom":
            ...
            if d["recipients"]=="teachers": qs=qs.filter(employee_type="teacher")
            contacts=[(e.full_name,e.phone) for e in qs]
        else:
            qs=Guardian.objects.filter(school=request.school)
            if d["recipients"]=="class":
                qs=qs.filter(student_links__student__enrollments__section=d["section"],
                             student_links__student__enrollments__academic_year__is_current=True).distinct()
