        ...
        for index, (name, cols, data) in enumerate([(title, headers, rows), *extra_sheets]):
            ...
            for col in ws.columns:
                ws.column_dimensions[col[0].column_letter].width = min(
                    45, max(12, max(len(str(c.value or "")) for c in col) + 2)
                )
        output = BytesIO()
        wb.save(output)
...
def pdf_response(title, headers, rows, subtitle=""):
    ...
    data = [
        [Paragraph(escape(str(v if v is not None else "")), styles["Normal"]) for v in row]
    ...
    doc.build(
        [Paragraph(escape(title), styles["Title"]), Paragraph(escape(subtitle), styles["Normal"]), Spacer(1, 15), table]
    )
