...
from .models import Exam,ExamSchedule,Mark,ResultSnapshot,UnlockRequest
from .grading import pct_of,grade_for,scale_rules
...
def assert_can_mark(user,schedule,enrollment=None):
    ...
    qs=SubjectTeacher.objects.filter(school=schedule.school,teacher=employee,subject=schedule.subject,
                                    academic_year=schedule.exam.academic_year,section__class_level=schedule.class_level)
    if enrollment: qs=qs.filter(section=enrollment.section)
...
def active_unlock(user,schedule):
    return UnlockRequest.objects.filter(schedule=schedule,requested_by=user,status="approved",expires_at__gt=timezone.now()).exists()
...
def save_mark(*,user,schedule,enrollment,score,absent=False,expected_version=0):
    ...
    if exam.status == "published":
        ...
        obj.save()
    AuditLog.objects.create(school=schedule.school,user=user,action="mark.updated",model=obj._meta.label,
        object_id=str(obj.pk),description=f"{before} -> {obj.marks_obtained}; absent={absent}; version={obj.version}")
    if exam.status=="published":
...
def live_class_sheet(exam,class_level):
    ...
    enrollments=list(Enrollment.objects.filter(school=exam.school,academic_year=exam.academic_year,class_level=class_level)
    ...
    for e in enrollments:
        ...
        for s in schedules:
            ...
            elif not passed: failed+=1
            total+=score
            points.append(Decimal(str(band["grade_point"])) if passed else Decimal(0))
            cells.append({"schedule_id":s.pk,"subject":s.subject.name,"full_marks":str(s.full_marks),
                          "pass_marks":str(s.pass_marks),"score":str(score) if not missing and not absent else None,
                          "missing":missing,"absent":absent,"percent":str(percent),"letter":band["letter"] if passed else "F",
                          "grade_point":str(points[-1]),"passed":passed})
        full=sum((s.full_marks for s in schedules),Decimal(0))
        gpa=(sum(points)/len(points)).quantize(Decimal("0.01")) if points and not failed and complete else Decimal(0)
        rows.append({"enrollment_id":e.pk,"student_id":e.student_id,"student_code":e.student.student_id,
                     "student":e.student.full_name,"section_id":e.section_id,"section":str(e.section),"class_level_id":class_level.pk,
                     "roll":e.roll_number,"cells":cells,"total":str(total),"full_total":str(full),
                     "percent":str(pct_of(total,full)) if complete else None,"gpa":str(gpa) if complete else None,
                     "result":"INCOMPLETE" if not complete else ("FAIL" if failed else "PASS"),"complete":complete})
...
def analyse(rows):
    ...
    summary={"appeared":len(complete),"incomplete":len(rows)-len(complete),
             "passed":sum(r["result"]=="PASS" for r in complete),"failed":sum(r["result"]=="FAIL" for r in complete),
             "average":round(sum(Decimal(r["percent"]) for r in complete)/len(complete),2) if complete else None}
    stats={}
    for row in rows:
        for c in row["cells"]:
            st=stats.setdefault(c["schedule_id"],{"subject":c["subject"],"entered":0,"absent":0,"missing":0,"passed":0,"scores":[]})
            if c["missing"]: st["missing"]+=1
            else:
                st["entered"]+=1
                if c["absent"]: st["absent"]+=1
                else: st["scores"].append(Decimal(c["score"]))
...
def build_result_sheet(exam,class_level,section=None):
    if exam.status=="published":
        rows=[dict(s.payload) for s in ResultSnapshot.objects.filter(exam=exam,version=exam.publication_version,
              payload__class_level_id=class_level.pk)]
    else:
        rows=live_class_sheet(exam,class_level)
    if section: rows=[r for r in rows if r["section_id"]==section.pk]
    rows.sort(key=lambda r:(r["grade_rank"] is None,r["grade_rank"] or 0,r["section"],r["roll"]))
    summary,stats=analyse(rows)
    return {"exam":exam,"class_level":class_level,"section":section,"rows":rows,"summary":summary,"subject_stats":stats}
...
def snapshot_exam(exam,user):
    ...
    for level in ClassLevel.objects.filter(pk__in=class_ids):
        ...
        if not rows or any(not r["complete"] for r in rows):
            raise ValidationError(f"{level}: every enrolled student needs marks or an explicit absence for each scheduled subject.")
        all_rows.extend(rows)
    version=exam.publication_version+1
    ResultSnapshot.objects.bulk_create([ResultSnapshot(school=exam.school,exam=exam,enrollment_id=r["enrollment_id"],version=version,payload=r) for r in all_rows])
    exam.status="published"
    exam.publication_version=version
    exam.published_by=user
    exam.published_at=timezone.now()
    exam.save()
    AuditLog.objects.create(school=exam.school,user=user,action="results.published",object_id=str(exam.pk),description=f"Snapshot version {version}")
    return exam
...
def review_unlock(request_obj,user,approve):
    ...
    obj.save()
    AuditLog.objects.create(school=obj.school,user=user,action="marks.unlock."+obj.status,object_id=str(obj.pk),description=obj.reason)
    return obj
