...
from core.access import assert_school, assert_actor_school
from core.models import School, AuditLog
from students.models import Enrollment
from .models import FeeInvoice, FeeInvoiceItem, FeeStructure, FeeConcession, FeePayment
...
def generate_invoices(*,school,user,academic_year,class_level,month,issue_date,due_date):
    ...
    structures=list(FeeStructure.objects.filter(school=school,academic_year=academic_year,class_level=class_level,category__is_active=True).select_related("category"))
    if not structures:
        raise ValidationError("Configure fees for this class and academic year first.")
    created=skipped=0
    for enr in Enrollment.objects.filter(school=school,academic_year=academic_year,class_level=class_level,
                                        student__status="active",status="enrolled").select_related("student"):
        ...
        for fs in structures:
            previous=FeeInvoiceItem.objects.filter(invoice__student=enr.student,category=fs.category).exclude(invoice__status="cancelled")
            ...
            if fs.frequency=="quarterly":
                quarter=(month-1)//3
                if previous.filter(invoice__academic_year=academic_year,invoice__month__in=range(quarter*3+1,quarter*3+4)).exists():
                    continue
            amount=fs.amount
            if amount<0:
        ...
        if not items:
            skipped+=1
            continue
        inv=FeeInvoice.objects.create(school=school,invoice_no=FeeInvoice.next_invoice_no(school,academic_year.name),
            student=enr.student,enrollment=enr,academic_year=academic_year,month=month,issue_date=issue_date,due_date=due_date,created_by=user)
        FeeInvoiceItem.objects.bulk_create([FeeInvoiceItem(invoice=inv,category=c,amount=a) for c,a in items])
        inv.refresh_status()
        created+=1
    AuditLog.objects.create(school=school,user=user,action="invoices.generated",description=f"{created} created; {skipped} skipped.")
...
def collect_payment(*,school,user,invoice,amount,method,reference,date):
    ...
    payment=FeePayment.objects.create(school=school,invoice=invoice,receipt_no=FeePayment.next_receipt_no(school),
        amount=amount,method=method,reference=reference,date=date,received_by=user)
    payment.post_to_ledger(user)
    invoice.refresh_status()
    AuditLog.objects.create(school=school,user=user,action="payment.collected",model=payment._meta.label,
                            object_id=str(payment.pk),description=f"{payment.receipt_no}: {amount}")
    return payment
...
def cancel_payment(*,school,user,payment,reason):
    ...
    AuditLog.objects.create(school=school,user=user,action="payment.cancelled",object_id=str(payment.pk),description=reason)
