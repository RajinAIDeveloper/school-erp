...
from .services import generate_invoices,collect_payment,cancel_payment

class InvoiceListView(ERPListView):
    model=FeeInvoice
    permission_required="fees.view_feeinvoice"
    page_title="Fee invoices"
    columns=(("Invoice","invoice_no"),("Student","student"),("Due date","due_date"),("Total","total","money"),("Paid","paid","money"),("Balance","balance","money"),("Status","status","badge"))
    ...
    extra_actions=(("Generate invoices","fees:generate"),("Fee categories","fees:category_list"),("Structures","fees:structure_list"),("Concessions","fees:concession_list"),("Collection report","fees:report"))
    ...
    if request.method=="POST":
        ...
        if form.is_valid():
            try:
                ...
                form.add_error(None,e)
    return render(request,"fees/detail.html",{"invoice":inv,"form":form,"page_title":inv.invoice_no,
                                           "items":inv.items.select_related("category"),"payments":inv.payments.all()})

@require_permission(None)
def receipt(request,pk):
    p=get_object_or_404(FeePayment,pk=pk,school=request.school,invoice__in=invoice_queryset(request))
    return pdf_response("Fee receipt "+p.receipt_no,["Invoice","Student","Date","Method","Amount","Status"],
        [[p.invoice.invoice_no,p.invoice.student.full_name,str(p.date),p.method,p.amount,"CANCELLED" if p.is_cancelled else "Received"]],
        request.school.name)
...
def report(request):
    ...
    if form.is_bound and form.is_valid():
        if form.cleaned_data["start"]: qs=qs.filter(date__gte=form.cleaned_data["start"])
        if form.cleaned_data["end"]: qs=qs.filter(date__lte=form.cleaned_data["end"])
    ...
    return render(request,"generic/report.html",{"page_title":"Fee collections","headers":headers,"rows":rows,"form":form})
