...
REPORTS=[
    ...
    ("Student attendance","attendance:student_report","attendance.view_studentattendance","Monthly grid and CSV / Excel"),
    ...
    ("Results and subject analysis","examinations:results","examinations.view_mark","Class and section rank, CSV / Excel / PDF"),
...
def hub(request):
    ...
    links=[{"name":name,"url":reverse(route),"description":description} for name,route,permission,description in REPORTS
           if request.user.has_perm(permission)]
    if request.user.has_perm("finance.view_account"):
        links.insert(0,{"name":"Management overview","url":reverse("reports:overview"),"description":"Date-range counts, attendance, fees and accounts."})
    if hasattr(request.user, "student_profile") or hasattr(request.user, "guardian_profile"):
        links = [
            {"name":"My attendance","url":reverse("portal:attendance"),"description":"Attendance history by month with CSV export."},
            {"name":"My fees","url":reverse("portal:fees"),"description":"Invoices, outstanding balances and receipts."},
            {"name":"My results","url":reverse("portal:results"),"description":"Published report cards for linked students."},
            *links,
        ]
...
def overview(request):
    ...
    if form.is_bound and form.is_valid():
        ...
        student_counts=Student.objects.filter(school=school).aggregate(total=Count("id"),active=Count("id",filter=Q(status="active")))
        staff_counts=Employee.objects.filter(school=school).aggregate(total=Count("id"),active=Count("id",filter=Q(status="active")))
        st=StudentAttendance.objects.filter(school=school,date__range=(start,end)).aggregate(total=Count("id"),present=Count("id",filter=Q(status__in=["present","late"])))
        emp=StaffAttendance.objects.filter(school=school,date__range=(start,end)).aggregate(total=Count("id"),present=Count("id",filter=Q(status__in=["present","late"])))
        payments=FeePayment.objects.filter(school=school,date__range=(start,end),is_cancelled=False).aggregate(total=Sum("amount"),count=Count("id"))
        dues=sum((i.balance for i in FeeInvoice.objects.filter(school=school,status__in=["unpaid","partial"])),Decimal("0"))
        income=sum((a.balance(start,end) for a in Account.objects.filter(school=school,account_type="income")),Decimal("0"))
        expenses=sum((a.balance(start,end) for a in Account.objects.filter(school=school,account_type="expense")),Decimal("0"))
        rows=[
        ...
        if fmt=="pdf": return pdf_response("School overview",["Metric","Value"],rows,f"{school.name} | {start} to {end}")
        if fmt in ("csv","xlsx"): return spreadsheet("school-overview",["Metric","Value"],rows,fmt)
